...
void
ParticleQPSOT1::CalculateNewVelocity(PREC* _global_best_params, PREC* _best_params_avg)
{
  ...
  for (COUNT_T i = 0; i < this->config.searching_dimension; i++)
  {
    ...
    const PREC velo = mixedBest + direction * (PREC)( this->Alpha * std::abs( p - mixedBest ) * log( 1 / rand ) );
